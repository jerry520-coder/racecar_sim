...
class odom_class:
    ...
    def __init__(self):
        ...
        # 订阅 关节
        rospy.Subscriber(
            joint_states_name, JointState, self.callback_join, queue_size=1
        )
        # 发布 odom 坐标
    ...
    def callback_join(self, data):
        ...
        # 平均速度
        velocity = (data.velocity[lrw_topic] + data.velocity[rrw_topic]) / (
            2 * 13.95348
        )
        # 平均路程
        ...
        if (self.yaw_angle[2] < 1.52) and (self.yaw_angle[2] > -1.52):
            self.odom_topic.twist.twist.linear.x = velocity * math.cos(
                self.yaw_angle[2]
            )
        else:
            self.odom_topic.twist.twist.linear.x = -velocity * math.cos(
